...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    if(isset($_POST['plant_name']) and isset($_POST['user_id']) and isset($_POST['buy_date']) 
            and isset($_POST['new_buy_location']) and isset($_POST['new_amount']) and isset($_POST['linked_device_id'])){
        // operate data
        $db = new DbOperator();
        $db->__contruct();
        $result = $db->changePlantSetting($_POST['user_id'], $_POST['plant_name'], $_POST['buy_date'], 
         $_POST['new_amount'], $_POST['new_buy_location'], $_POST['linked_device_id']);
        if($result == 1){
